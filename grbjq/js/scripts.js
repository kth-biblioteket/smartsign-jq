$(document).ready(function() {
    ...
    function getroomAvailability(area_id){
        ...
        var html = '';
        $.ajax({
            url: '/webservices/mrbs/api/v1/noauth/roomsavailability/?timestamp=' + currenttimestamp+ '&limit=1000&area_id=' + area_id + '&database=mrbs_grupprum&appname=mrbsgrupprum' ,
            /*
            success: function(){
            },
            error: function(){
...
$(document).ajaxSuccess(function (event, jqxhr, settings) {  
    ...
    if (settings.url.indexOf("area_id=1") != -1) {
        ...
        roomsfiltered.forEach(function(element) {
            ...
            i++;
            html += '<div class="' + colorclass +' Smartsign-item flex-container"><div>' + element.room_number + '. ' + element.room_name + '</div></div>';
        }); 
        html += '</div>';
        //uppdatera html
