...
$api = file_get_contents('https://cloud.timeedit.net/kth/web/public01/ri.json?h=f&sid=9&p=' . $todaysdate . '-' . $todaysdate . '&objects=417156.4&ox=0&types=0&fe=0&g=f&pl=f&sec=t&h2=2)');
...
foreach ($lokalbokningarray as $bokning){
    array_push($timeditarray['reservations'], 
        array('id' => $bokning['Event_ID'],
                "startdate" => $bokning['Start_Date'],
                "starttime" => $bokning['Start_Time'],
                "enddate"=>$bokning['End_Date'],
                "endtime"=>$bokning['End_Time'],
                "columns"=>array($bokning['Event_Title'],
                $bokning['Event_Object'],"","","")));
}
*/
...
$timeditarray['properties'] = array('ipaddress' => $_SERVER['REMOTE_ADDR']);
